...
function ExportDailyOrder($orders)
{
    ...
    //$fileName = "\\\\xasudc01\\Data\\PWIN17P\\PWCIMPORT\\DailyOrders.csv";
    header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
    header('Content-Description: File Transfer');
    header("Content-type: text/csv");
    header("Content-Disposition: attachment; filename={$fileName}");
...
`weborder`.`time`,
`weborder`.`po`,
`weborder`.`cost_centre`,
`weborder`.`status`,
`weborder`.`print_status`,
`weborder`.`delivered`,
`weborder`.`bill_first_name`,
...
`weborder`.`total_freight`,
`weborder`.`tax_freight`,
`weborder`.`client_freight_account`,
`weborder`.`client_freight_name`,
`weborder`.`client_freight_flag`,
`weborder`.`instructions`,
`weborder`.`instructions2`,
`weborder`.`username`,
`weborder`.`due_date`,
`weborder`.`backorder`,
`weborder_item`.`id_product`,
`weborder_item`.`sku`,
`weborder_item`.`qty`,
...
`weborder_item`.`price`,
`weborder_item`.`total` As 'LineTotal',
`weborder_item`.`address_id`
...
WHERE  `date` = CURDATE() ";
		...
		{*/
			...
			$data[] = $order['tax'];
			$data[] = $order['total'];
			$data[] = $order['estimate_freight'];
			$data[] = $order['total_freight'];
			$data[] = $order['tax_freight'];
			$data[] = $order['instructions'];
			$data[] = $order['instructions2'];
			$data[] = $order['username'];
			$data[] = $order['due_date'];
			fputcsv($fh, $data);
		/*}*/
							...
							while ($row = $result->fetchRow())
							{
								if (! empty($row['sku']))
								{
									...
									if($sku->save()){
										...
										mail('Zhilong.Liang@vavavoom.com','Error Saving SKU',print_r($sku,1));
									}
	...
	else
	{
		...
		if($input->post['markAsPrinted'])
		{
			if($input->post['printOrder'])
			{
				...
				$_SESSION['printpackingsliplist'] = $orderToBePrinted_array;
				//header("Location:/webadmin/logistics/all_orders");
				// no header("Location:/webadmin/logistics/all_orders_with_packing_slip");
...
//WARREN PLATYBANKEXPORT
		...
		$sql = "SELECT * FROM weborder_item wi
				JOIN weborder w ON w.id_order=wi.id_order
				JOIN vvv_client c ON c.id_client = w.id_client
                WHERE w.date = NOW()";
			$result = $db->read($sql);
			...
			/* for staging test on old db < DATE_ADD(NOW(), INTERVAL +1 MONTH)  */
        }
	}
}
...
// echo $sql;
	...
	if($result->num_rows > 0 || $result_a->num_rows > 0)
	{
		...
		if(isset($_SESSION['username'])) {		
			if(in_array(strtolower($_SESSION['username']), array('richard.karora@vavavoom.com','jason.jackson@vavavoom.com','webadmin@brandmanager.biz'))) {
