...
function ExportDailyOrder($orders)
{
    ...
    foreach ( $orders as $order ) 
    {
		...
		{*/
			...
			$data[] = $order['ship_postcode'];
			$data[] = $order['freight'];
			$data[] = $order['estimate_freight']; //non nz
			$data[] = $order['surcharge'];
			$data[] = $order['bill_email'];
			fputcsv($fh, $data);
		/*}*/
    }
...
//WARREN PLATYBANKEXPORT
        ...
        else if ($input->post['exportToDailyOrder'])
        {
			...
			$clause .= " w.print_status = 'yes' ORDER BY w.date`";

			$sql = "SELECT wi.qty, wi.sku, wi.name,wi.price as unit_price, wi.size,wi.colour,wi.unit_size,w.*,c.name as client_name, c.id_client as client_id FROM weborder_item wi
			JOIN weborder w ON w.id_order=wi.id_order
			JOIN vvv_client c ON c.id_client = w.id_client 
			WHERE  w.status = 'placed' AND w.date >= CURDATE() - 4	";// w.date >= CURDATE()"; //AND w.print_status = 'yes'  w.status = 'placed' AND
			$result = $db->read($sql);
			$orders = $result->resultSetToArray();				
